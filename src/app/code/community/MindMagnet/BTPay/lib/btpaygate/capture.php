...
try {
    ...
    var_dump($payment_gateway->getConfigData());
    
	$capture = $payment_gateway->capture($payment,'http://www.mindmagnetsoftware.com/btpaygate/capture.php',$_GET['rrn'],$_GET['intref']);
    
    var_dump($capture);
	if ($capture->isValid($config_data['encryption_key'])) {
		//psign is valid, verify response
		echo "VALID RESPONSE ... RESULTCODE = ".$capture->getCaptureResult()."<br/>".PHP_EOL;
	} else {
